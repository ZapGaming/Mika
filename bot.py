...
# Migrating to commands.Bot to handle slash commands properly.
...
async def get_gemini_response(prompt_text: str, channel_id: int) -> str:
    ...
    ai_persona_instruction = (
        ...
        "**Proactively engage if a conversation seems natural for you to join, even without a direct mention, but don't interrupt or spam.** " # Slight nudge for "roaming freely" within context
...
async def on_ready():
    ...
    load_chat_history() # Load history on startup.
...
async def on_message(message: discord.Message):
    ...
    # --- 2. PROCESS AI CHAT ---
    # Mika responds to DMs, mentions, or general messages in servers
    # IF they are not commands AND Mika is mentioned.
    # For general "roaming freely", the AI prompt encourages proactive but contextual engagement.
    
    # Ignore if the message starts with a command character (like '/')
    if message.content.startswith('/'):
        return # Let the command handler manage it.
    
    # Mika should respond if she's mentioned or if it's a DM.
...
async def chat_command(ctx, *, user_input: str):
    """
    Triggers a chat interaction with Mika. This is the primary way to explicitly chat.
    """
...
async def embed_command(ctx, url: str):
    """
    Generates a themed link embed for a provided URL.
    """
    print(f"Received /embed command from {ctx.author.display_name} with URL: {url}")
    
    # Add a check to ensure the input looks like a URL.
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandNotFound):
        # Ignore unknown commands, no need to notify the user.
        pass 
    elif isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f"Oopsie! You seem to be missing a part of the command. Try `/help {ctx.command}` to see how to use it correctly! 💖")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("That argument format seems a bit off, darling. Check `/help` to make sure you're using it right! ✨")
    elif isinstance(error, commands.CommandInvokeError):
        # Catches errors happening inside the command itself.
        print(f"Error executing command {ctx.command}: {error.original}")
        await ctx.send("Mika stumbled a little while processing that command! Maybe try again in a moment? 🥺")
    else:
        # Log any other unhandled errors for debugging.
        print(f"Unhandled command error: {error}")
        await ctx.send("Oh no! Something unexpected happened. Let's try that command again, maybe? ✨")


# --- RUN THE BOT ---
if __name__ == "__main__": 
    try:
        # Load history BEFORE running the bot client.
        load_chat_history() 
        
        # Run the bot using bot.run() instead of client.run()
