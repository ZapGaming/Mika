...
import json               
# --- SPOTIFY LIBRARY ---
import spotipy            # For Spotify API interactions
...
if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
    # Inform if Spotify creds are missing but don't necessarily block bot startup,
    # as it might still function without Spotify features. However, log it.
    print("WARNING: SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRET not found. Spotify search commands will not work.")
    # You might want to conditionally enable/disable Spotify features based on this check.
...
bot = commands.Bot(command_prefix='!', intents=intents) # Use bot instance
...
# --- SPOTIPY CLIENT ---
# Initialize spotipy client; will only work if credentials are provided.
sp = None
if SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET:
    try:
        ...
        print(f"Failed to initialize Spotify client: {e}. Spotify features will be unavailable.")
else:
    print("Spotify credentials not found. Spotify search will be unavailable.")
...
async def get_gemini_response(prompt_text: str, channel_id: int) -> str:
    ...
    ai_persona_instruction = (
        ...
        "Proactively engage if a conversation seems natural for you to join, even without a direct mention, but don't interrupt or spam. "
...
async def on_message(message: discord.Message):
    ...
    # --- 2. PROCESS AI CHAT (for mentions/DMs, and potentially contextual responses) ---
    # Don't process chat if the message was a slash command (starts with '/').
    if message.content.startswith('/'):
        return 
    
    # Mika responds to DMs or if she is mentioned. The AI prompt also encourages contextual engagement.
...
# --- SPOTIFY COMMAND ---
@bot.command(name='spotify', help='Search for a song on Spotify. Example: /spotify Bohemian Rhapsody Queen')
async def spotify_search(ctx, *, query: str):
    """Searches for a song on Spotify and returns results in a themed embed."""
    print(f"Received /spotify command from {ctx.author.display_name} with query: '{query}'")

    if not sp: # Check if spotipy client was successfully initialized.
        await ctx.send("Mika isn't quite connected to Spotify yet! Her music-finding module seems a bit offline. 😔 Please check the bot's status or developer.")
        return

    try:
        # Search for tracks. Limit to 1 result for simplicity.
        results = sp.search(q=query, type='track', limit=1)

        if results and results['tracks'] and results['tracks']['items']:
            track = results['tracks']['items'][0] # Get the first track result.

            # Extracting information for the embed
            track_name = track['name']
            artists = ", ".join([artist['name'] for artist in track['artists']])
            album_name = track['album']['name']
            track_url = track['external_urls'].get('spotify') # Get Spotify URL for the track.
            # Album art URL (prefer higher resolution if available)
            thumbnail_url = track['album']['images'][0]['url'] if track['album']['images'] else None 
            # Preview URL - Spotify might not always provide one
            preview_url = track.get('preview_url')

            # --- Creating a Themed Embed for Spotify Results ---
            embed = discord.Embed(
                title=f"🎵 {track_name}", # Track name in title with music emoji.
                description=f"**By:** {artists}\n**Album:** {album_name}", # Display artists and album.
                url=track_url, # Link the entire embed to the Spotify track page.
                color=SELECTED_EMBED_COLOR # Use Mika's signature theme color.
            )

            if thumbnail_url: # Set thumbnail to album art if available.
                embed.set_thumbnail(url=thumbnail_url)
            
            # Add a footer for attribution and thematic flair.
            footer_text = f"Searched by: {ctx.author.display_name} | ✨ Spotify Power ✨"
            footer_icon_url = ctx.author.avatar.url if ctx.author.avatar else bot.user.default_avatar.url
            embed.set_footer(text=footer_text, icon_url=footer_icon_url)

            # Add a field for preview URL if it exists.
            if preview_url:
                embed.add_field(name="🎧 Preview Available", value=f"[Listen](https://open.spotify.com/embed/track/{track_url.split('/')[-1]}?utm_source=generator) (May require Spotify account for full playback)", inline=False)
                # Note: Embedding Spotify player directly might require specific bot permissions/setup
                # For now, providing a link to the preview or track page is standard.
                # A direct embed might look like: embed.add_field(name="Listen", value=f"[Play Preview]({preview_url})", inline=False)
                # However, Discord embeds don't always render rich previews for direct audio links perfectly.
                # Let's provide a safe Spotify link for the track itself.

            await ctx.send(embed=embed) # Send the created embed to the channel.

        else:
            # If no results were found for the query.
            await ctx.send(f"Hmph! Mika couldn't find any songs matching '{query}'. Maybe try a different search term? 😉🎶")

    except Exception as e:
        # Catch any unexpected errors during the Spotify search process.
        print(f"Error during Spotify search for query '{query}': {e}")
        await ctx.send(f"Oh dear, something went wrong trying to search Spotify for '{query}'. Mika's music connection might be a little fuzzy right now! 😔")
